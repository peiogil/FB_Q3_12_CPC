...
case ON_BUCK2:
    ...
    else
    {
        if( cont_RX_bits<11)
        {
            ...
            {
            while (DataRdyUART1()!=1);
                FlybackVoltagePID.abcCoefficients[cont_abcCoef]=U1RXREG;
                FlybackVoltagePID.abcCoefficients[cont_abcCoef]=FlybackVoltagePID.abcCoefficients[cont_abcCoef]<<8;
                FlybackVoltagePID.abcCoefficients[cont_abcCoef]=FlybackVoltagePID.abcCoefficients[cont_abcCoef]|parametroL;
                cont_abcCoef++;
            }
            cont_RX_bits++;
            while(BusyUART1());	
...
{
        while (DataRdyUART1()!=1);
FlybackReferenceNew=U1RXREG;
FlybackReferenceNew=FlybackReferenceNew<<8;
FlybackReferenceNew=(FlybackReferenceNew|referenciaVoltageL);
FlybackVoltagePID.controlReference=FlybackReferenceNew;
cont_RX_bits=0;
ordenUSB=NO_ORDEN;
}
...
outputVoltageL=(char)(FlybackVoltagePID.measuredOutput>>2);
outputVoltageH=(char)(FlybackVoltagePID.measuredOutput>>10);
}
else
